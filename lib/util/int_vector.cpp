...
static void int_vector_realloc_data__(int_vector_type *vector,
                                      int new_alloc_size, int default_value) {
    if (new_alloc_size != vector->alloc_size) {
        if (vector->data_owner) {
            if (new_alloc_size > 0) {
                int i;
                vector->data = (int *)util_realloc(
...
static int_vector_type *int_vector_alloc__(int init_size, int default_value,
                                           int *data, int alloc_size,
                                           bool data_owner) {
...
int_vector_type *int_vector_alloc(int init_size, int default_value) {
    int_vector_type *vector =
        int_vector_alloc__(init_size, default_value, NULL, 0, true);
    return vector;
}
    ...
    if (new_size > vector->size) {
        if (new_size > vector->alloc_size) {
            for (int i = vector->size; i < vector->alloc_size; i++)
                vector->data[i] = default_value;
            int_vector_realloc_data__(vector, 2 * new_size, default_value);
        } else
...
int_vector_type *int_vector_alloc_shared_wrapper(int init_size,
                                                 int default_value, int *data,
                                                 int alloc_size) {
    return int_vector_alloc__(init_size, default_value, data, alloc_size,
                              false);
}
...
int_vector_type *int_vector_alloc_private_wrapper(int init_size,
                                                  int default_value, int *data,
                                                  int alloc_size) {
    return int_vector_alloc__(init_size, default_value, data, alloc_size,
                              false);
...
/**
   ...
   fail hard.
*/
void int_vector_memcpy_data_block(int_vector_type *target,
                                  const int_vector_type *src, int target_offset,
...
void int_vector_memcpy_from_data(int_vector_type *target, const int *src,
                                 int src_size) {
    int_vector_reset(target);
    //int_vector_iset( target , src_size - 1 , 0 );
    int_vector_resize(target, src_size, 0);
...
*/
int_vector_type *int_vector_alloc_strided_copy(const int_vector_type *src,
                                               int start, int stop,
                                               int stride) {
...
int_vector_type *int_vector_alloc_copy(const int_vector_type *src) {
    ...
    int_vector_realloc_data__(copy, src->alloc_size, src->default_value);
    copy->size = src->size;
...
void int_vector_iset_default(int_vector_type *vector, int index,
                             int default_value) {
...
/* vector + vector */
void int_vector_inplace_add(int_vector_type *vector,
                            const int_vector_type *delta) {
...
/* vector - vector */
void int_vector_inplace_sub(int_vector_type *vector,
                            const int_vector_type *delta) {
    if (vector->size != delta->size)
        util_abort("%s: combining vectors with different size: %d and %d \n",
                   __func__, vector->size, delta->size);
...
void int_vector_inplace_mul(int_vector_type *vector,
                            const int_vector_type *factor) {
...
/* vector / vector (elementwise) */
void int_vector_inplace_div(int_vector_type *vector,
                            const int_vector_type *inv_factor) {
...
void int_vector_iset_block(int_vector_type *vector, int index, int block_size,
                           int value) {
...
void int_vector_append(int_vector_type *vector, int value) {
    ...
    int_vector_resize(vector, size + 1, 0);
...
void int_vector_free_container(int_vector_type *vector) { free(vector); }

void int_vector_free_data(int_vector_type *vector) {
    int_vector_reset(vector);
    int_vector_realloc_data__(vector, 0, vector->default_value);
}
...
int int_vector_size(const int_vector_type *vector) { return vector->size; }
...
void int_vector_set_many(int_vector_type *vector, int index, const int *data,
                         int length) {
    int_vector_assert_writable(vector);
    {
...
*/
void int_vector_init_range(int_vector_type *vector, int value1, int value2,
                           int delta) {
    int_vector_reset(vector);
    if (delta != 0) {
...
bool int_vector_init_linear(int_vector_type *vector, int start_value,
                            int end_value, int num_values) {
    if (num_values < 2)
        return false;

    int_vector_reset(vector);
    int_vector_iset(vector, 0, start_value);
    {
        double slope = (end_value - start_value) / (num_values - 1);

        for (int i = 1; i < num_values - 1; i++) {
            int value = (int)start_value + slope * i;
            int_vector_iset(vector, i, value);
        }
    }
    int_vector_iset(vector, num_values - 1, end_value);
    return true;
}

void int_vector_append_many(int_vector_type *vector, const int *data,
                            int length) {
    int_vector_set_many(vector, int_vector_size(vector), data, length);
}

void int_vector_append_vector(int_vector_type *vector,
                              const int_vector_type *other) {
    int_vector_append_many(vector, int_vector_get_const_ptr(other),
                           int_vector_size(other));
}
...
void int_vector_shrink(int_vector_type *vector) {
    int_vector_realloc_data__(vector, vector->size, vector->default_value);
}
...
int_vector_alloc_sort_perm__(const int_vector_type *vector, bool reverse) {
...
void int_vector_permute(int_vector_type *vector, const perm_vector_type *perm) {
    int_vector_assert_writable(vector);
    {
        int i;
        int *tmp =
            (int *)util_alloc_copy(vector->data, sizeof *tmp * vector->size);
        for (i = 0; i < vector->size; i++)
...
int int_vector_lookup_bin__(const int_vector_type *limits, int value,
                            int guess) {
    if (guess >= 0) {
...
void int_vector_fprintf(const int_vector_type *vector, FILE *stream,
...
void int_vector_fread_data(int_vector_type *vector, int size, FILE *stream) {
    int_vector_realloc_data__(vector, size, vector->default_value);
...
bool int_vector_equal(const int_vector_type *vector1,
                      const int_vector_type *vector2) {
...
int int_vector_first_equal(const int_vector_type *vector1,
                           const int_vector_type *vector2, int offset) {
...
int int_vector_first_not_equal(const int_vector_type *vector1,
                               const int_vector_type *vector2, int offset) {
    if (offset >= vector1->size)
        return -2;
...
void int_vector_range_fill(int_vector_type *vector, int limit1, int delta,
                           int limit2) {
