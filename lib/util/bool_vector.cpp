...
static void bool_vector_realloc_data__(bool_vector_type *vector,
                                       int new_alloc_size, bool default_value) {
    if (new_alloc_size != vector->alloc_size) {
        if (vector->data_owner) {
            if (new_alloc_size > 0) {
                int i;
                vector->data = (bool *)util_realloc(
...
static void bool_vector_memmove(bool_vector_type *vector, int offset,
                                int shift) {
    if ((shift + offset) < 0)
...
static void bool_vector_assert_index(const bool_vector_type *vector,
                                     int index) {
...
static bool_vector_type *bool_vector_alloc__(int init_size, bool default_value,
...
bool_vector_type *bool_vector_alloc(int init_size, bool default_value) {
    bool_vector_type *vector =
        bool_vector_alloc__(init_size, default_value, NULL, 0, true);
    return vector;
}
    ...
    if (new_size > vector->size) {
        if (new_size > vector->alloc_size) {
            for (int i = vector->size; i < vector->alloc_size; i++)
                vector->data[i] = default_value;
            bool_vector_realloc_data__(vector, 2 * new_size, default_value);
        } else
...
bool_vector_type *bool_vector_alloc_shared_wrapper(int init_size,
                                                   bool default_value,
                                                   bool *data, int alloc_size) {
    return bool_vector_alloc__(init_size, default_value, data, alloc_size,
                               false);
}
...
void bool_vector_memcpy_data_block(bool_vector_type *target,
    ...
    if (target->alloc_size < (target_offset + len))
        bool_vector_iset(target, target_offset + len - 1,
...
void bool_vector_memcpy_from_data(bool_vector_type *target, const bool *src,
    ...
    bool_vector_resize(target, src_size, 0);
...
*/
bool_vector_type *bool_vector_alloc_strided_copy(const bool_vector_type *src,
                                                 int start, int stop,
                                                 int stride) {
...
bool_vector_type *bool_vector_alloc_copy(const bool_vector_type *src) {
    ...
    bool_vector_realloc_data__(copy, src->alloc_size, src->default_value);
    copy->size = src->size;
...
/* vector + vector */
void bool_vector_inplace_add(bool_vector_type *vector,
...
void bool_vector_inplace_mul(bool_vector_type *vector,
...
void bool_vector_idel_block(bool_vector_type *vector, int index,
                            int block_size) {
    bool_vector_assert_writable(vector);
    {
...
void bool_vector_append(bool_vector_type *vector, bool value) {
    ...
    bool_vector_resize(vector, size + 1, 0);
...
void bool_vector_free_container(bool_vector_type *vector) { free(vector); }

void bool_vector_free_data(bool_vector_type *vector) {
    bool_vector_reset(vector);
    bool_vector_realloc_data__(vector, 0, vector->default_value);
}
...
int bool_vector_size(const bool_vector_type *vector) { return vector->size; }
...
void bool_vector_set_many(bool_vector_type *vector, int index, const bool *data,
                          int length) {
    bool_vector_assert_writable(vector);
    {
...
bool bool_vector_init_linear(bool_vector_type *vector, bool start_value,
    ...
    bool_vector_reset(vector);
    bool_vector_iset(vector, 0, start_value);
    {
        double slope = (end_value - start_value) / (num_values - 1);

        for (int i = 1; i < num_values - 1; i++) {
            bool value = (bool)start_value + slope * i;
            bool_vector_iset(vector, i, value);
        }
    }
    bool_vector_iset(vector, num_values - 1, end_value);
    return true;
}
...
void bool_vector_append_vector(bool_vector_type *vector,
    ...
    bool_vector_append_many(vector, bool_vector_get_const_ptr(other),
                            bool_vector_size(other));
}
...
void bool_vector_shrink(bool_vector_type *vector) {
    bool_vector_realloc_data__(vector, vector->size, vector->default_value);
}
...
bool_vector_alloc_sort_perm__(const bool_vector_type *vector, bool reverse) {
    ...
    if (reverse)
        qsort(sort_nodes, vector->size, sizeof *sort_nodes,
              bool_vector_rcmp_node);
    else
        qsort(sort_nodes, vector->size, sizeof *sort_nodes,
              bool_vector_cmp_node);
...
void bool_vector_permute(bool_vector_type *vector,
    ...
    bool_vector_assert_writable(vector);
    {
        int i;
        bool *tmp =
            (bool *)util_alloc_copy(vector->data, sizeof *tmp * vector->size);
        for (i = 0; i < vector->size; i++)
...
void bool_vector_fprintf(const bool_vector_type *vector, FILE *stream,
...
void bool_vector_fread_data(bool_vector_type *vector, int size, FILE *stream) {
    bool_vector_realloc_data__(vector, size, vector->default_value);
