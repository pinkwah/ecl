...
static void double_vector_realloc_data__(double_vector_type *vector,
                                         int new_alloc_size,
    ...
    if (new_alloc_size != vector->alloc_size) {
        if (vector->data_owner) {
            if (new_alloc_size > 0) {
                int i;
                vector->data = (double *)util_realloc(
...
static void double_vector_memmove(double_vector_type *vector, int offset,
                                  int shift) {
    if ((shift + offset) < 0)
...
static void double_vector_assert_index(const double_vector_type *vector,
                                       int index) {
...
static double_vector_type *double_vector_alloc__(int init_size,
                                                 double default_value,
                                                 double *data, int alloc_size,
                                                 bool data_owner) {
    double_vector_type *vector =
        (double_vector_type *)util_malloc(sizeof *vector);
...
double_vector_type *double_vector_alloc(int init_size, double default_value) {
    double_vector_type *vector =
        double_vector_alloc__(init_size, default_value, NULL, 0, true);
    return vector;
}
...
double_vector_type *double_vector_alloc_shared_wrapper(int init_size,
                                                       double default_value,
                                                       double *data,
                                                       int alloc_size) {
    return double_vector_alloc__(init_size, default_value, data, alloc_size,
                                 false);
}
...
double_vector_type *double_vector_alloc_private_wrapper(int init_size,
                                                        double default_value,
                                                        double *data,
                                                        int alloc_size) {
    return double_vector_alloc__(init_size, default_value, data, alloc_size,
...
/**
   ...
   fail hard.
*/
void double_vector_memcpy_data_block(double_vector_type *target,
                                     const double_vector_type *src,
    ...
    if (target->alloc_size < (target_offset + len))
        double_vector_iset(target, target_offset + len - 1,
...
void double_vector_memcpy_from_data(double_vector_type *target,
                                    const double *src, int src_size) {
...
void double_vector_memcpy(double_vector_type *target,
                          const double_vector_type *src) {
    double_vector_reset(target);
...
*/
double_vector_type *
double_vector_alloc_strided_copy(const double_vector_type *src, int start,
                                 int stop, int stride) {
...
double_vector_type *double_vector_alloc_copy(const double_vector_type *src) {
    double_vector_type *copy =
        double_vector_alloc(src->size, src->default_value);
...
void double_vector_set_default(double_vector_type *vector,
                               double default_value) {
...
void double_vector_append_default(double_vector_type *vector,
                                  double default_value) {
...
void double_vector_iset_default(double_vector_type *vector, int index,
                                double default_value) {
...
/* vector + vector */
void double_vector_inplace_add(double_vector_type *vector,
                               const double_vector_type *delta) {
...
/* vector - vector */
void double_vector_inplace_sub(double_vector_type *vector,
                               const double_vector_type *delta) {
    if (vector->size != delta->size)
        util_abort("%s: combining vectors with different size: %d and %d \n",
                   __func__, vector->size, delta->size);
...
void double_vector_inplace_mul(double_vector_type *vector,
                               const double_vector_type *factor) {
...
/* vector / vector (elementwise) */
void double_vector_inplace_div(double_vector_type *vector,
                               const double_vector_type *inv_factor) {
...
void double_vector_iset_block(double_vector_type *vector, int index,
                              int block_size, double value) {
...
void double_vector_idel_block(double_vector_type *vector, int index,
                              int block_size) {
    double_vector_assert_writable(vector);
    {
...
void double_vector_free_container(double_vector_type *vector) { free(vector); }
...
void double_vector_set_many(double_vector_type *vector, int index,
...
*/
void double_vector_init_range(double_vector_type *vector, double value1,
                              double value2, double delta) {
    double_vector_reset(vector);
    if (delta != 0) {
...
bool double_vector_init_linear(double_vector_type *vector, double start_value,
...
void double_vector_append_many(double_vector_type *vector, const double *data,
                               int length) {
    double_vector_set_many(vector, double_vector_size(vector), data, length);
}

void double_vector_append_vector(double_vector_type *vector,
                                 const double_vector_type *other) {
    double_vector_append_many(vector, double_vector_get_const_ptr(other),
                              double_vector_size(other));
}
...
int double_vector_get_max_index(const double_vector_type *vector,
                                bool reverse) {
    if (vector->size == 0)
...
int double_vector_get_min_index(const double_vector_type *vector,
                                bool reverse) {
    if (vector->size == 0)
...
bool double_vector_contains_sorted(const double_vector_type *vector,
                                   double value) {
...
double_vector_alloc_sort_perm__(const double_vector_type *vector,
                                bool reverse) {
    ...
    if (reverse)
        qsort(sort_nodes, vector->size, sizeof *sort_nodes,
              double_vector_rcmp_node);
    else
        qsort(sort_nodes, vector->size, sizeof *sort_nodes,
              double_vector_cmp_node);
...
perm_vector_type *
double_vector_alloc_sort_perm(const double_vector_type *vector) {
    return double_vector_alloc_sort_perm__(vector, false);
}

perm_vector_type *
double_vector_alloc_rsort_perm(const double_vector_type *vector) {
    return double_vector_alloc_sort_perm__(vector, true);
}

void double_vector_permute(double_vector_type *vector,
                           const perm_vector_type *perm) {
    double_vector_assert_writable(vector);
    {
        int i;
        double *tmp =
            (double *)util_alloc_copy(vector->data, sizeof *tmp * vector->size);
        for (i = 0; i < vector->size; i++)
...
int double_vector_lookup_bin(const double_vector_type *limits, double value,
                             int guess) {
...
int double_vector_lookup_bin__(const double_vector_type *limits, double value,
                               int guess) {
    if (guess >= 0) {
...
void double_vector_fprintf(const double_vector_type *vector, FILE *stream,
...
*/
void double_vector_fread_data(double_vector_type *vector, int size,
                              FILE *stream) {
...
bool double_vector_equal(const double_vector_type *vector1,
                         const double_vector_type *vector2) {
...
int double_vector_first_equal(const double_vector_type *vector1,
                              const double_vector_type *vector2, int offset) {
...
int double_vector_first_not_equal(const double_vector_type *vector1,
                                  const double_vector_type *vector2,
                                  int offset) {
    if (offset >= vector1->size)
        return -2;
...
int double_vector_count_equal(const double_vector_type *vector,
                              double cmp_value) {
    int count = 0;
    int i;
    for (i = 0; i < vector->size; i++)
...
void double_vector_range_fill(double_vector_type *vector, double limit1,
                              double delta, double limit2) {
