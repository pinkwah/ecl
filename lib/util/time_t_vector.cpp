...
static void time_t_vector_realloc_data__(time_t_vector_type *vector,
                                         int new_alloc_size,
    ...
    if (new_alloc_size != vector->alloc_size) {
        if (vector->data_owner) {
            if (new_alloc_size > 0) {
                int i;
                vector->data = (time_t *)util_realloc(
...
static void time_t_vector_memmove(time_t_vector_type *vector, int offset,
                                  int shift) {
    if ((shift + offset) < 0)
...
static void time_t_vector_assert_index(const time_t_vector_type *vector,
                                       int index) {
...
static time_t_vector_type *time_t_vector_alloc__(int init_size,
                                                 time_t default_value,
                                                 time_t *data, int alloc_size,
                                                 bool data_owner) {
    time_t_vector_type *vector =
        (time_t_vector_type *)util_malloc(sizeof *vector);
...
time_t_vector_type *time_t_vector_alloc(int init_size, time_t default_value) {
    time_t_vector_type *vector =
        time_t_vector_alloc__(init_size, default_value, NULL, 0, true);
    return vector;
}
...
time_t_vector_type *time_t_vector_alloc_shared_wrapper(int init_size,
                                                       time_t default_value,
                                                       time_t *data,
                                                       int alloc_size) {
    return time_t_vector_alloc__(init_size, default_value, data, alloc_size,
                                 false);
}
...
time_t_vector_type *time_t_vector_alloc_private_wrapper(int init_size,
                                                        time_t default_value,
                                                        time_t *data,
                                                        int alloc_size) {
    return time_t_vector_alloc__(init_size, default_value, data, alloc_size,
...
/**
   ...
   fail hard.
*/
void time_t_vector_memcpy_data_block(time_t_vector_type *target,
                                     const time_t_vector_type *src,
                                     int target_offset, int src_offset,
                                     int len) {
    ...
    if (target->alloc_size < (target_offset + len))
        time_t_vector_iset(target, target_offset + len - 1,
...
void time_t_vector_memcpy_from_data(time_t_vector_type *target,
                                    const time_t *src, int src_size) {
...
void time_t_vector_memcpy(time_t_vector_type *target,
                          const time_t_vector_type *src) {
    time_t_vector_reset(target);
...
*/
time_t_vector_type *
time_t_vector_alloc_strided_copy(const time_t_vector_type *src, int start,
                                 int stop, int stride) {
...
time_t_vector_type *time_t_vector_alloc_copy(const time_t_vector_type *src) {
    time_t_vector_type *copy =
        time_t_vector_alloc(src->size, src->default_value);
...
void time_t_vector_set_default(time_t_vector_type *vector,
                               time_t default_value) {
...
void time_t_vector_append_default(time_t_vector_type *vector,
                                  time_t default_value) {
...
void time_t_vector_iset_default(time_t_vector_type *vector, int index,
                                time_t default_value) {
...
/* vector + vector */
void time_t_vector_inplace_add(time_t_vector_type *vector,
                               const time_t_vector_type *delta) {
...
/* vector - vector */
void time_t_vector_inplace_sub(time_t_vector_type *vector,
                               const time_t_vector_type *delta) {
    if (vector->size != delta->size)
        util_abort("%s: combining vectors with different size: %d and %d \n",
                   __func__, vector->size, delta->size);
...
void time_t_vector_inplace_mul(time_t_vector_type *vector,
                               const time_t_vector_type *factor) {
...
/* vector / vector (elementwise) */
void time_t_vector_inplace_div(time_t_vector_type *vector,
                               const time_t_vector_type *inv_factor) {
                       ...
                       __func__, index);
        {
            if (vector->alloc_size <= index)
                time_t_vector_realloc_data__(
                    vector, 2 * (index + 1),
                    vector
                        ->default_value); /* Must have ( + 1) here to ensure we are not doing 2*0 */
...
void time_t_vector_iset_block(time_t_vector_type *vector, int index,
                              int block_size, time_t value) {
...
void time_t_vector_idel_block(time_t_vector_type *vector, int index,
                              int block_size) {
    time_t_vector_assert_writable(vector);
    {
...
void time_t_vector_free_container(time_t_vector_type *vector) { free(vector); }
...
void time_t_vector_set_many(time_t_vector_type *vector, int index,
                            const time_t *data, int length) {
    time_t_vector_assert_writable(vector);
    {
...
*/
void time_t_vector_init_range(time_t_vector_type *vector, time_t value1,
                              time_t value2, time_t delta) {
    time_t_vector_reset(vector);
    if (delta != 0) {
...
bool time_t_vector_init_linear(time_t_vector_type *vector, time_t start_value,
                               time_t end_value, int num_values) {
    if (num_values < 2)
        return false;
...
void time_t_vector_append_many(time_t_vector_type *vector, const time_t *data,
                               int length) {
    time_t_vector_set_many(vector, time_t_vector_size(vector), data, length);
}

void time_t_vector_append_vector(time_t_vector_type *vector,
                                 const time_t_vector_type *other) {
    time_t_vector_append_many(vector, time_t_vector_get_const_ptr(other),
                              time_t_vector_size(other));
}
...
int time_t_vector_get_max_index(const time_t_vector_type *vector,
                                bool reverse) {
    if (vector->size == 0)
...
int time_t_vector_get_min_index(const time_t_vector_type *vector,
                                bool reverse) {
    if (vector->size == 0)
...
bool time_t_vector_contains_sorted(const time_t_vector_type *vector,
                                   time_t value) {
...
static perm_vector_type *
time_t_vector_alloc_sort_perm__(const time_t_vector_type *vector,
                                bool reverse) {
    ...
    if (reverse)
        qsort(sort_nodes, vector->size, sizeof *sort_nodes,
              time_t_vector_rcmp_node);
    else
        qsort(sort_nodes, vector->size, sizeof *sort_nodes,
              time_t_vector_cmp_node);
...
perm_vector_type *
time_t_vector_alloc_sort_perm(const time_t_vector_type *vector) {
    return time_t_vector_alloc_sort_perm__(vector, false);
}

perm_vector_type *
time_t_vector_alloc_rsort_perm(const time_t_vector_type *vector) {
    return time_t_vector_alloc_sort_perm__(vector, true);
}

void time_t_vector_permute(time_t_vector_type *vector,
                           const perm_vector_type *perm) {
    time_t_vector_assert_writable(vector);
    {
        int i;
        time_t *tmp =
            (time_t *)util_alloc_copy(vector->data, sizeof *tmp * vector->size);
        for (i = 0; i < vector->size; i++)
...
int time_t_vector_lookup_bin(const time_t_vector_type *limits, time_t value,
                             int guess) {
...
int time_t_vector_lookup_bin__(const time_t_vector_type *limits, time_t value,
                               int guess) {
    if (guess >= 0) {
...
void time_t_vector_fprintf(const time_t_vector_type *vector, FILE *stream,
...
*/
void time_t_vector_fread_data(time_t_vector_type *vector, int size,
                              FILE *stream) {
...
bool time_t_vector_equal(const time_t_vector_type *vector1,
                         const time_t_vector_type *vector2) {
...
int time_t_vector_first_equal(const time_t_vector_type *vector1,
                              const time_t_vector_type *vector2, int offset) {
...
int time_t_vector_first_not_equal(const time_t_vector_type *vector1,
                                  const time_t_vector_type *vector2,
                                  int offset) {
    if (offset >= vector1->size)
        return -2;
...
int time_t_vector_count_equal(const time_t_vector_type *vector,
                              time_t cmp_value) {
    int count = 0;
    int i;
    for (i = 0; i < vector->size; i++)
...
void time_t_vector_range_fill(time_t_vector_type *vector, time_t limit1,
                              time_t delta, time_t limit2) {
